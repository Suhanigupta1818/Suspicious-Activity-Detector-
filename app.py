import streamlit as st
import tensorflow as tf
import cv2
import numpy as np
import smtplib
import os
from email.message import EmailMessage
from datetime import datetime
import time
from tensorflow.keras.layers import InputLayer

# --- 1. KERAS 3 COMPATIBILITY PATCH (IMPORTANT) ---
class PatchedInputLayer(InputLayer):
    def __init__(self, *args, **kwargs):
        kwargs.pop('batch_shape', None)
        kwargs.pop('optional', None)
        super().__init__(*args, **kwargs)

# --- CONFIGURATION ---
SENDER_EMAIL = "gsuhani053@gmail.com"
SENDER_PASSWORD = "kuem fujp pnmz oyxe" 
RECEIVER_EMAIL = "gsuhani053@gmail.com"

# --- PAGE SETUP ---
st.set_page_config(page_title="AI Sentinel | Security", page_icon="🛡️", layout="wide")

st.markdown("""
    <style>
    .main { background-color: #f5f7f9; }
    .stButton>button { width: 100%; border-radius: 5px; height: 3em; background-color: #007bff; color: white; font-weight: bold; }
    h1 { color: #1e3d59; }
    </style>
    """, unsafe_allow_html=True)

# --- SIDEBAR ---
with st.sidebar:
    st.image("https://cdn-icons-png.flaticon.com/512/2564/2564344.png", width=100)
    st.title("AI Sentinel Control")
    st.info("Status: System Online ✅")
    st.divider()
    st.write("**Model:** MobileNetV2 + LSTM")

# --- 2. LOAD MODEL (WITH SCOPE) ---
@st.cache_resource
def load_my_model():
    model_path = 'final_improved_model.keras'
    if not os.path.exists(model_path):
        # Agar .keras nahi milti toh .h5 try karo
        model_path = 'final_improved_model.h5'
    
    if not os.path.exists(model_path):
        st.error("Model file missing on GitHub!")
        return None
    try:
        # Scope use karne se 'InputLayer' error 100% solve ho jata hai
        with tf.keras.utils.custom_object_scope({'InputLayer': PatchedInputLayer}):
            return tf.keras.models.load_model(model_path, compile=False)
    except Exception as e:
        st.error(f"Final Loading Error: {e}")
        return None

model = load_my_model()

# --- FUNCTIONS ---
def send_email_alert(confidence):
    try:
        current_time = datetime.now().strftime("%I:%M %p | %d %b %Y")
        msg = EmailMessage()
        msg.set_content(f"🚨 SECURITY ALERT 🚨\n\nSuspicious activity detected.\nConfidence: {confidence:.2f}%")
        msg['Subject'] = f"⚠️ CRITICAL ALERT: Security"
        msg['From'] = SENDER_EMAIL
        msg['To'] = RECEIVER_EMAIL
        server = smtplib.SMTP_SSL('smtp.gmail.com', 465)
        server.login(SENDER_EMAIL, SENDER_PASSWORD)
        server.send_message(msg)
        server.quit()
        return True
    except: return False

def extract_frames(video_path, num_frames=15, img_size=(64, 64)):
    frames = []
    cap = cv2.VideoCapture(video_path)
    total_frames = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    if total_frames < num_frames: return None
    indices = np.linspace(0, total_frames - 1, num_frames, dtype=int)
    for idx in indices:
        cap.set(cv2.CAP_PROP_POS_FRAMES, idx)
        ret, frame = cap.read()
        if ret:
            frame = cv2.resize(frame, img_size)
            frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            frames.append(frame)
    cap.release()
    return np.array(frames)

# --- MAIN UI ---
st.title("🛡️ Smart Surveillance Dashboard")
uploaded_file = st.file_uploader("Upload surveillance footage", type=["mp4", "avi"])

if uploaded_file:
    with open("temp_video.mp4", "wb") as f:
        f.write(uploaded_file.read())
    st.video("temp_video.mp4")

    if st.button("START THREAT SCAN"):
        if model is None:
            st.error("Model not loaded!")
        else:
            frames = extract_frames("temp_video.mp4")
            if frames is not None:
                test_input = np.expand_dims(frames, axis=0).astype('float32') / 255.0
                res = model.predict(test_input)
                prediction = float(np.squeeze(res))
                
                if prediction > 0.5:
                    st.error(f"⚠️ SUSPICIOUS ACTIVITY ({prediction*100:.2f}%)")
                    send_email_alert(prediction*100)
                else:
                    st.success(f"✅ NORMAL BEHAVIOR ({(1-prediction)*100:.2f}%)")
            else:
                st.warning("Video too short.")

st.caption("Developed by Suhani | 2026")
